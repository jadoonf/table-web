        ...
        <div className="mt-16 justify-center gap-6 sm:grid sm:grid-cols-2 sm:space-y-0 lg:grid-cols-3">
          {plans.map((item, idx) => (
            ...
            >
              ...
              <div className={ny("animate-background-shine p-8 space-y-4 border-b" ,  item.name === 'Enterprise' ? "dark:bg-[linear-gradient(110deg,transparent,45%,#d6cfcb,55%,transparent)] bg-[length:200%_100%] transition-colors rounded-t-2xl" : "")}>
                <span className="text-neutral-600 font-normal font-geist tracking-tight">{item.name}</span>
                <div className="text-gray-400 dark:text-gray-200 text-3xl font-semibold">
                 ${item.price}{" "}
                  <span className="text-xl text-gray-900 dark:text-gray-200  font-normal">/mo</span>
                </div>
                <p>{item.desc}</p>
